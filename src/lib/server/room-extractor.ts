...
export function extractVRBORoomsAndBeds(html: string): ExtractedRoom[] {
	...
	const bedroomPatterns = [
		...
		/(?:bedroom|room)\s*(\d+)[^<]*?(\d+)\s*(king|queen|twin|full|bunk|sofa|murphy)[^<]*bed/gi,
		/<[^>]*>bedroom\s*(\d+)[^<]*?(\d+)\s*(king|queen|twin|full|bunk|sofa|murphy)[^<]*bed/gi
	];
...
export async function extractPropertyRoomsAndPhotos(
	...
	try {
		...
		return extractRoomsAndPhotosFromHTML(html, isVRBO ? 'vrbo' : 'airbnb');
	} catch (error) {
		console.error('Error fetching HTML for room extraction:', error);
		return { rooms: [], photos: [] };
	}
}
