<script lang="ts">
	...
	const expectedGuests = $derived(Math.max(1, draft.expectedGuestCount || 1));
	const perPersonShare = $derived(BASE_PRICE / expectedGuests);
	...
	async function createTrip(isPublished: boolean): Promise<string> {
		const res = await fetch('/api/trips/publish', {
			method: 'POST',
			headers: { 'Content-Type': 'application/json' },
			body: JSON.stringify({ ...draft, isPublished, splitCost })
		});
	...
	async function handleSaveDraft() {
		...
		publishError = null;
		try {
			const tripId = await createTrip(false);
			tripDraft.clear();
			goto(`/trips/${tripId}`);
		} catch (e) {
			publishError = e instanceof Error ? e.message : 'Something went wrong. Please try again.';
	...
	const ctaLabel = $derived(() => {
		...
		return finalPrice === 0 ? 'Publish Trip' : 'Publish & Pay';
	});
</script>
...
<div class="publish-screen">
	...
	<div class="content-grid">
		<!-- Left column: pricing + discount + split -->
		<div class="left-col">
			<!-- Pricing Card -->
			...
			<div class="card split-card">
				...
				{#if splitCost}
					<div class="split-details">
						...
						<div class="split-detail-row">
							<span class="split-detail-label">Based on</span>
							<span class="split-detail-value">{expectedGuests} guest{expectedGuests !== 1 ? 's' : ''}</span>
						</div>
						<p class="split-note">
							We'll automatically factor this into your trip pricing so each guest contributes a small portion.
							If your expected guest count is reached, your total cost will be fully covered.
						</p>
					</div>
				{/if}
			</div>
		...
		<div class="right-col">
			<div class="card summary-card">
				...
				{#if splitCost && expectedGuests > 1}
					<div class="summary-line muted-line">
						<span>Split across {expectedGuests} guests</span>
